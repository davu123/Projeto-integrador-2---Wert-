...
export default function UsersPage() {
  ...
  async function carregarUsuarios() {
    try {
      ...
      setErro(error?.response?.data?.message || 'Erro ao carregar usuários.');
    } finally {
      setLoading(false);
    }
  }
    ...
    <div style={{ padding: '24px' }}>
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ margin: 0, color: '#64748b' }}>
          Gerencie administradores, técnicos, gestores e auditores.
        </p>
      </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'grid', gap: '14px' }}>
            ...
            <div>
              ...
              <input
                ...
                value={form.email}
                onChange={handleChange}
                placeholder="email@wert.com.br"
                style={inputStyle}
              />
            </div>

            <div>
              <label>{editandoId ? 'Nova senha (opcional)' : 'Senha'}</label>
              <input
                name="senha"
                type="password"
                value={form.senha}
                ...
                <tbody>
                  {usuarios.map((usuario) => (
                    <tr key={usuario.id}>
                      ...
                      <td style={tdStyle}>{usuario.perfil}</td>
                      <td style={tdStyle}>
                        <span
                          style={{
                            padding: '6px 10px',
