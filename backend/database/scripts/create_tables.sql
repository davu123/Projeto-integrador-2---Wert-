...
CREATE TABLE dbo.usuario (
    ...
    CONSTRAINT CK_usuario_perfil
        CHECK (perfil IN ('administrador', 'tecnico', 'gestor', 'auditor'))
);
GO
...
CREATE TABLE dbo.lote (
    ...
    CONSTRAINT FK_lote_agencia
        FOREIGN KEY (agencia_id) REFERENCES dbo.agencia(id),

    CONSTRAINT FK_lote_tecnico
        FOREIGN KEY (tecnico_id) REFERENCES dbo.usuario(id),

    CONSTRAINT CK_lote_status
        CHECK (status IN ('pendente', 'em_triagem', 'concluido'))
);
GO
...
CREATE TABLE dbo.equipamento (
    ...
    CONSTRAINT FK_equipamento_lote
        FOREIGN KEY (lote_id) REFERENCES dbo.lote(id),

    CONSTRAINT UQ_equipamento_numero_serie
        UNIQUE (numero_serie),

    CONSTRAINT CK_equipamento_estado
        CHECK (estado IN ('bom', 'danificado', 'inutilizavel'))
);
GO
